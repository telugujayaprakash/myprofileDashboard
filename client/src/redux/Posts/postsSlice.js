...
export const fetchFeedPosts = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth('http://localhost:3000/api/posts/feed', {
        method: 'GET'
      });
...
export const fetchUserPosts = createAsyncThunk(
  ...
  async (username, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`http://localhost:3000/${username}/posts`, {
        method: 'GET'
      });
...
export const createPost = createAsyncThunk(
  ...
  async (postData, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth('http://localhost:3000/api/posts', {
        method: 'POST',
...
export const likePost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`http://localhost:3000/api/posts/${postId}/like`, {
        method: 'PUT'
      });
...
export const sharePost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`http://localhost:3000/api/posts/${postId}/share`, {
        method: 'PUT'
      });
