...
export const searchUsers = createAsyncThunk(
  ...
  async (query, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`${import.meta.env.VITE_BASE_URL}/api/search/users?q=${encodeURIComponent(query)}`, {
        method: 'GET'
      });
...
export const searchPosts = createAsyncThunk(
  ...
  async (query, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`${import.meta.env.VITE_BASE_URL}/api/search/posts?q=${encodeURIComponent(query)}`, {
        method: 'GET'
      });
