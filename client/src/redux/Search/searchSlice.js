...
export const searchUsers = createAsyncThunk(
  ...
  async (query, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`http://localhost:3000/api/search/users?q=${encodeURIComponent(query)}`, {
        method: 'GET'
      });
...
export const searchPosts = createAsyncThunk(
  ...
  async (query, { rejectWithValue }) => {
    try {
      const response = await fetchWithAuth(`http://localhost:3000/api/search/posts?q=${encodeURIComponent(query)}`, {
        method: 'GET'
      });
